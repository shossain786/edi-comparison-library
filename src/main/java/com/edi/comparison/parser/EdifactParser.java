...
/**
 * Parser for EDIFACT (Electronic Data Interchange For Administration, Commerce and Transport) files.
 * 
 * <p>EDIFACT format characteristics:
 * <ul>
 *   <li>Segment delimiter: ' (apostrophe)</li>
 *   <li>Element delimiter: + (plus)</li>
 *   <li>Component delimiter: : (colon)</li>
 *   <li>Example: BGM+340+BOOKING123+9'</li>
 * </ul>
 * 
...
public class EdifactParser implements FileParser {
    ...
    // Pattern to extract message type from UNH segment
    private static final Pattern MESSAGE_TYPE_PATTERN = Pattern.compile("UNH\\+[^+]*\\+([^:+]+)");
    ...
    public Message parse(String content) throws ParseException {
        ...
        for (String segmentStr : segments) {
            ...
            try {
                ...
                if (segment.getTag().equals("UNH") && messageType == null) {
                    messageType = extractMessageType(trimmed);
                }
    ...
    public boolean canParse(String content) {
        ...
        // EDIFACT typically starts with UNB or UNH
        String trimmed = content.trim();
        return trimmed.startsWith("UNB") || trimmed.startsWith("UNH") || 
               trimmed.contains("UNH+") || content.contains("'");
    }
    
    /**
     * Parses a single segment string into a Segment object.
     * 
     * @param segmentStr raw segment string (e.g., "BGM+340+BOOKING123+9")
    ...
    private Segment parseSegment(String segmentStr, int lineNumber, int sequenceNumber) {
        ...
        for (int i = 1; i < elements.length; i++) {
            ...
            // Check if element has components (separated by :)
            if (element.contains(":")) {
                String[] components = element.split(DEFAULT_COMPONENT_DELIMITER);
                for (int j = 0; j < components.length; j++) {
                    String position = String.format("%s.C%03d.%04d", tag, i, j + 1);
     ...
     * Example: UNH+1+IFTMBF:D:96A:UN' -> returns "IFTMBF"
     * 
     * @param unhSegment UNH segment string
     * @return message type or null if not found
     */
    private String extractMessageType(String unhSegment) {
        Matcher matcher = MESSAGE_TYPE_PATTERN.matcher(unhSegment);
        if (matcher.find()) {
